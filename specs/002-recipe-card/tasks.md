...
- [x] T017 [P] [US3] Imagen プロバイダラッパを実装（遅延生成、gemini.ts に倣う）（src/server/model/imagen.ts）
- [x] T018 [US3] イラスト生成サービス（`experimental_generateImage` で生成のみ。画像バイトと MIME を返す）を実装（src/server/services/illustration.ts）
