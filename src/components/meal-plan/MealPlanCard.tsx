...
// LLM が「○日目」を料理名に含めることがあるため除去する
const stripDaySuffix = (title: string) =>
  title.replace(/[\s\u3000]*[(（]\d+日目[)）]/g, "").trim()
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-3">
        {mealPlan.meals.map((meal) => {
          const title = stripDaySuffix(meal.title)
          return (
            <div
              key={meal.day}
            ...
            >
              <div className="flex items-baseline justify-between gap-2">
                ...
                <SaveRecipeButton
                  ...
                  isSaved={savedTitles?.has(normalize(title)) ?? false}
                  onSave={onSaveRecipe}
                />
              </div>
