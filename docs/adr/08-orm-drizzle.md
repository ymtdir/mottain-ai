...
ORM / DB アクセスに Drizzle ORM を採用する。マイグレーション SQL の生成（`drizzle-kit generate`）と適用（`drizzle-kit migrate`）はいずれも drizzle-kit が担い、別途マイグレーションツールは導入しない。Postgres ドライバは postgres.js（`postgres`）を用いる。スキーマは `src/server/db/schema.ts` に定義する。

### 理由

- 軽量・SQL 寄り・型安全で、スキーマ定義から型付きクエリまで単体で完結する。別 ORM は不要。
- マイグレーションも drizzle-kit 単体で完結する（`generate` で SQL ファイルを生成、`migrate` で適用）ため、Flyway・golang-migrate などのツールを追加する必要がない。
...
- スキーマを `src/server/db/schema.ts` に集約し、`drizzle-kit generate` で移行 SQL を生成・`drizzle-kit migrate` で適用、postgres.js で接続する。
