...
version: alpha
name: mottain-ai
description: にんじんテーマ。食材を余らせない献立エージェントの、暖かく親しみのある家庭的な UI
colors:
...
mottain-ai は「冷蔵庫の食材を余らせない献立」を AI エージェントとのチャットで提案する家庭向けアプリ。UI が目指す感情は、台所に立つ人の隣に寄り添うような**暖かさ・親しみ・素朴さ**であり、洗練よりも生活感を優先する。
...
Zen Maru Gothic（丸ゴシック）を全ロールで用いる。角のない柔らかい字形で「暖かく親しみのある」トーンを文字レベルから支える。見出しも本文も同じファミリで統一し、素朴で読みやすい印象を保つ。見出し（h1/h2）はミディアム〜セミボールドのウェイトでリズムをつけ、本文（body）は行間 1.5 で可読性を確保する。補助情報は label・caption で明度を落として扱う。基準フォントサイズは `html { font-size: 106% }` でわずかに底上げし、視認性を高める。
...
角丸は `--radius: 1rem` を lg の基準とし、sm（0.6rem）・md（0.8rem）を派生させる。やや大きめの角丸を基調にして、鋭さを避け親しみのある柔らかい形にする。
